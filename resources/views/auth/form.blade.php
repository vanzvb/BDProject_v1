...
@section('content')
    ...
    @foreach($questions as $question)
        {{-- Check if followup_question_text exists and display it above the question --}}
        ...
        <div class="card mb-3">
            <div class="card-header">
                {{-- Show the question's order number --}}
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    {{-- Handle different input types based on question type --}}
                    @if($question->type == 'text')
                        <input type="text" name="question_{{ $question->id }}" id="question_{{ $question->id }}" class="form-control" {{ $question->is_mandatory ? 'required' : '' }}>
                    @elseif($question->type == 'textarea')
                        <textarea name="question_{{ $question->id }}" id="question_{{ $question->id }}" class="form-control" {{ $question->is_mandatory ? 'required' : '' }}></textarea>
                    @elseif($question->type == 'select')
                        <select name="question_{{ $question->id }}" id="question_{{ $question->id }}" class="form-control" {{ $question->is_mandatory ? 'required' : '' }}>
                            <option value="">-- Select --</option>
                        </select>
                    @elseif($question->type == 'checkbox')
                        <input type="checkbox" name="question_{{ $question->id }}" id="question_{{ $question->id }}" {{ $question->is_mandatory ? 'required' : '' }}>
                    @elseif($question->type == 'radio')
                        <div>
                            <input type="radio" name="question_{{ $question->id }}" value="yes" {{ $question->is_mandatory ? 'required' : '' }}> Yes
                            <input type="radio" name="question_{{ $question->id }}" value="no" {{ $question->is_mandatory ? 'required' : '' }}> No
                        </div>
                    @endif
                </div>
            </div>
